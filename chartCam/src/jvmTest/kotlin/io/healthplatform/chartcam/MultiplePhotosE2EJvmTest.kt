...
class MultiplePhotosE2EJvmTest {
    ...
    fun testCreateAndCaptureMultiplePhotos() =
        runTest(testDispatcher) {
            ...
            // 3. Simulate capturing ONLY photo 2 using a specific linkId
            ...
            val targetedSteps = allSteps.filter { it.id == targetLinkId }

            targetedCaptureViewModel.initSteps(targetedSteps)

            assertEquals(1, targetedSteps.size)
            assertEquals(
                "Photo 2 Label",
                targetedCaptureViewModel.uiState.value.currentStep
                    ?.title,
            )
        }
}
